...
\item ``identification logically precedes estimation, inference, and testing.'' %(\href{https://www.aeaweb.org/articles?id=10.1257/jel.20181361}{Lewbel 2019}) 
\item A model is or is not point identified. (cf. ``better identification'')
\end{itemize}
...
\end{frame}
% -----------------------------------------
\begin{frame}{Given the trade elasticity, trade flows are sufficient}
\begin{itemize}
\item
...
\\ \textcolor{gray}{$\tau_{ii} = 1$ normalization does presume $X_{ii} > 0$ \\ obviously the parameter vector is not unique}
\item
In other words, this is a ``saturated'' model:
$N(N-1)$ degrees of freedom in $\tau_{ij}$
accommodate $N(N-1)$ observable outcomes
without any restrictions
\item[] \textcolor{gray}{Trade matrix has $N^2$ elements. Why is one row redundant?}
\item 
Bottom line: Any trade matrix $\{X_{ij}\}_{ij}$ can be rationalized by bilateral trade costs
\end{itemize}
\end{frame}
% -----------------------------------------
...
\begin{itemize}
\item
So far, we use only trade volumes (values), not distinguishing prices and quantities
\item
...
\end{frame}
% -----------------------------------------
\begin{frame}{Given the trade elasticity, trade shares are sufficient}
\begin{itemize}
\item
Given trade flows data \textcolor{gray}{(including the diagonal)},
you observe the sufficient statistics
...
\end{frame}
% -----------------------------------------
\begin{frame}{Dingel and Tintelnot (2025)}
...
Go to Dingel and Tintelnot (2025) slidedeck.
First half of paper concerns this overfitting problem.
Second half of paper introduces a finite model to quantify uncertainty.
\end{frame}
% -----------------------------------------
\end{document}

...
\note{
save unreasonable properties for exact hat algebra in models with intermediate inputs a la Rodrigo for EHA week
}
...
\note{
	Some comments on the relationship between international trade and economic geography.
	I work at the intersection. So do Redding, Arkolakis, Donaldson, Allen, Rossi-Hansberg and friends.
	Many of our good job candidates (Nick T, Elisa G, etc) learned trade tools and wrote geography JMPs.
}
\end{frame}
% -----------------------------------------
...
\note{
	\begin{itemize}
		\item Harrigan (page 28):
		{\small ``Equation (29) simply says that productivity advantage in a sector is associated with export success in that sector,
		a prediction that arises from any number of partial-equilibrium supply and demand models (for example, a simple Cournot reciprocal dumping model predicts that export success will be negatively related to marginal cost).
		The critique is not that (29) is inconsistent with a Ricardian model, but that verification of $> 0$ is not evidence in favor of Ricardo over any other explanation.
		A truly general equilibrium prediction of Ricardian models is that a productivity advantage in one sector can actually hurt export success in another sector, but Golub and Hsieh do not investigate this prediction.''}
	\end{itemize}
}
\end{frame}
% -----------------------------------------
...
\note{Perfect competition: MC=MR=AR=AC}
\end{frame}
% -----------------------------------------
...
\end{itemize}
\note{
	It's an endowment economy. A trade model without a production function: no comparative advantage, no returns to scale.
}
\end{frame}
% -----------------------------------------
...
\note{
You want to iterate on a contraction. 

Look at Allen and Arkolakis's section on ``6.4 Computing the Equilibrium (Alvarez-Lucas)''

Alvarez and Lucas:
``country $i$'s labor excess demand per unit of labor, so the operator $M$ prescribes that the percentage increase in country $i$'s wage be in proportion to a scaled version of country $i$'s excess demand.''

Pretty sure that $\sigma \geq 1$ suffices for the gross-substitutes property. $\frac{\partial f_i(\mathbf{w})}{\partial w_k} > 0 \ \forall k \neq i $}
...
\note{
	There is no reason to rewrite from $A_i$ to $T_i$ on this slide.
	I'm previewing a connection between this simple and other models that David Weinstein will teach you.
}
\end{frame}
% -----------------------------------------
